...
class sysmGAN(object):
    def __init__(self, g_net, h_net, dx_net, dx_net_cat, dy_net, x_sampler, y_sampler, batch_size, nb_classes, alpha, beta):
        ...
        self.pool = ImagePool()
        self.use_L1=False
        self.x_dim = self.g_net.input_dim
        self.y_dim = self.h_net.input_dim
        ...
        self.y_onehot = tf.placeholder(tf.float32, [None, self.nb_classes], name='y_onehot')

        self.y_ = self.g_net(self.x,reuse=False)
        self.x__ = self.h_net(self.y_,reuse=False)

        self.x_ = self.h_net(self.y)
        self.x_combine_ = tf.concat([self.x_,self.y_onehot],axis=1)
    
        self.y__ = self.g_net(self.x_)

        self.dy = self.dy_net(self.y,reuse=False)
        self.dy_ = self.dy_net(self.y_)

        self.dx = self.dx_net(self.x_combine,reuse=False)
        self.dx_ = self.dx_net(self.x_combine_)
        ...
        self.recon_x = tf.train.AdamOptimizer(learning_rate=self.lr, beta1=0.5, beta2=0.9) \
                .minimize(self.l2_loss_x, var_list=self.g_net.vars+self.h_net.vars)
    ...
    def train(self, epochs=1000):
        ...
        onehot_y = np.eye(self.nb_classes)[label_y]
        ...
        for epoch in range(epochs):
            ...
            lr = 1e-3 #if epoch < epochs/2 else 1e-4#2e-4*(epochs-epoch)/(epochs-epochs/2)
            ...
            for idx in range(batch_idxs):
                ...
                by_onehot = onehot_y[batch_size*idx:batch_size*(idx+1)]
                ...
                _ = self.sess.run(self.recon_y, feed_dict={self.x: bx, self.x_onehot: bx_onehot, self.y: by, self.y_onehot: by_onehot, self.lr:lr})
                _ = self.sess.run(self.dis_x, feed_dict={self.x: bx, self.x_onehot: bx_onehot, self.y: by, self.y_onehot: by_onehot, self.lr:lr})
                _ = self.sess.run(self.gen_x, feed_dict={self.x: bx, self.x_onehot: bx_onehot, self.y: by, self.y_onehot: by_onehot, self.lr:lr})
                _ = self.sess.run(self.recon_x, feed_dict={self.x: bx, self.x_onehot: bx_onehot, self.y: by, self.y_onehot: by_onehot, self.lr:lr})
                _ = self.sess.run(self.dis_y, feed_dict={self.x: bx, self.x_onehot: bx_onehot, self.y: by, self.y_onehot: by_onehot, self.lr:lr})
                _ = self.sess.run(self.gen_y, feed_dict={self.x: bx, self.x_onehot: bx_onehot, self.y: by, self.y_onehot: by_onehot, self.lr:lr})
                ...
                if counter % 100 == 0:
                    ...
                    h_loss_adv, g_loss_adv, l2_loss_x, l2_loss_y, dx_loss, \
                        dy_loss= self.sess.run(
                        [self.gx_loss, self.gy_loss, self.l2_loss_x, self.l2_loss_y, \
                        self.dx_loss, self.dy_loss],
                        feed_dict={self.x: bx, self.x_onehot:bx_onehot, self.y: by,self.y_onehot: by_onehot}
                    )
                    print('Iter [%8d] Time [%5.4f] g_loss_adv [%.4f] h_loss_adv [%.4f] l2_loss_x [%.4f] \
                        l2_loss_y [%.4f] dx_loss [%.4f] dy_loss [%.4f]' %
    ...
    def evaluate(self,timestamp,epoch,run_kmeans=False):
        ...
        #infer label with gmm given data_x_
        label_infer = self.x_sampler.predict_multipoints(data_x_)
        ...
        accuracy = accuracy_score(label_y,label_infer)     
        print('Rountrip: Accuracy = {}, Purity = {}, NMI = {}, ARI = {}'.format(accuracy,purity,nmi,ari))
        save_dir = 'data/density_est/classify_{}_{}_{}_{}_{}'.format(self.timestamp,self.x_dim, self.y_dim, self.alpha, self.beta)
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        #np.savez('{}/data_at_{}.npz'.format(save_dir, epoch+1),data_x,data_y,data_x_,data_y_)
        np.savez('{}/data_at_{}.npz'.format(save_dir, epoch+1),data_x_)
...
if __name__ == '__main__':
    ...
    g_net = model.Generator(x_dim, y_dim, name='g_net')
    #g_net = model.Generator(x_dim, y_dim, 'g_net')
    h_net = model.Generator(y_dim, x_dim, name='h_net')
    #h_net = model.Encoder(y_dim, x_dim+nb_classes, x_dim, 'h_net')
    dx_net = model.Discriminator(x_dim+nb_classes, 'dx_net')
    ...
    x_onehot = tf.placeholder(tf.float32, [None, nb_classes], name='x_onehot')
    x_combine = tf.concat([x,x_onehot],axis=1)

    y = tf.placeholder(tf.float32, [None, y_dim], name='y')
    y_onehot = tf.placeholder(tf.float32, [None, nb_classes], name='y_onehot')
