...
class sysmGAN(object):
    def __init__(self, g_net, h_net, dx_net, dy_net, x_sampler, y_sampler, batch_size, nb_classes, alpha, beta):
        self.model = model
        self.data = data
        self.g_net = g_net
        self.h_net = h_net
        ...
        self.x_, self.x_onehot_, self.x_logits_ = self.h_net(self.y)
    ...
    def train(self, epochs=800):
        ...
        for epoch in range(epochs):
            ...
            lr = 0.0001 #if epoch < epochs/2 else 2e-4*(epochs-epoch)/(epochs-epochs/2)
            ...
            for idx in range(batch_idxs):
                ...
                if counter % 100 == 0:
                    ...
                    dx_loss, dy_loss, d_loss = self.sess.run([self.dx_loss, self.dy_loss, self.d_loss], \
                        feed_dict={self.x: bx, self.x_onehot: bx_onehot, self.y: by, self.fake_x: fake_bx, self.fake_x_onehot:fake_bx_onehot, self.fake_y: fake_by})
    ...
    def evaluate(self,timestamp,epoch):
        ...
        #k-means
        if (epoch+1) % 100 ==0:
            km = KMeans(n_clusters=10, random_state=0).fit(data_y)
...
if __name__ == '__main__':
    ...
    xs = util.Mixture_sampler(nb_classes=nb_classes,N=10000,dim=x_dim)
    ...
    ys = util.UniformDataSampler(N=10000,n_components=10,dim=y_dim,sd=0.5)
    ...
    cl_gan = sysmGAN(g_net, h_net, dx_net, dy_net, xs, ys, batch_size, nb_classes, alpha, beta)
