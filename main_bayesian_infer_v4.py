#in the two stage training, its a RTM training + GAN training
#suit for non time-series data with independent distribution
...
class RoundtripModel(object):
    def __init__(self, g_net, h_net, dx_net1, dx_net2, dy_net1,dy_net2, x_sampler1, x_sampler2, y_sampler, pool, batch_size, alpha, beta, sd_y, df, scale):
        self.model = model
        self.data = data
        self.g_net = g_net
        self.h_net = h_net
        ...
        self.df = df
        self.scale = scale
        self.x_dim1 = self.dx_net1.input_dim
        self.x_dim2 = self.dx_net2.input_dim-self.x_dim1
        self.y_dim1 = self.dy_net1.input_dim
        self.y_dim2 = self.dy_net2.input_dim-self.y_dim1
        tf.reset_default_graph()
        ##########################################################################
        '''
            x1,y1 - hidden variable and obversation data
            x2,y2 - anonther hidden variable and model parameters(e.g.,theta)
        ...
        self.dy_ = self.dy_net2(self.y_, reuse=False)
        self.dx1_ = self.dx_net1(self.x1_, reuse=False)
        self.dx_ = self.dx_net2(self.x_, reuse=False)
        ...
        self.dx = self.dx_net2(self.x)
        self.dy = self.dy_net2(self.y)
        ...
        self.d_fake_x = self.dx_net2(self.fake_x)
        self.d_fake_y = self.dy_net2(self.fake_y)
        #-D(x)
        ...
        self.dx_loss = (tf.reduce_mean((0.9*tf.ones_like(self.dx) - self.dx)**2) \
                +tf.reduce_mean((0.1*tf.zeros_like(self.d_fake_x) - self.d_fake_x)**2))/2.0
        #self.dy_loss = (tf.reduce_mean((0.9*tf.ones_like(self.dy) - self.dy)**2) \
        #        +tf.reduce_mean((0.1*tf.zeros_like(self.d_fake_y) - self.d_fake_y)**2))/2.0
        #log(D(x))
        ...
        #stage2 GAN((x1,x2),(y1,y2))
        self.g_h_optim = tf.train.AdamOptimizer(learning_rate=self.lr, beta1=0.5, beta2=0.9) \
                .minimize(self.g_loss_adv, var_list=self.g_net.vars[1])
        self.d_optim = tf.train.AdamOptimizer(learning_rate=self.lr, beta1=0.5, beta2=0.9) \
                .minimize(self.dy_loss, var_list=self.dy_net2.vars)


        # self.g_h_optim = tf.train.AdamOptimizer(learning_rate=self.lr, beta1=0.5, beta2=0.9) \
        #         .minimize(self.g_h_loss, var_list=self.g_net.vars[1]+self.h_net.vars[1])
        # self.d_optim = tf.train.AdamOptimizer(learning_rate=self.lr, beta1=0.5, beta2=0.9) \
        #         .minimize(self.d_loss, var_list=self.dx_net2.vars+self.dy_net2.vars)
        ...
        self.graph_dir = 'graph/bayes_infer_{}_x_dim={}_{}_y_dim={}_{}_alpha={}_beta={}_sd={}_df={}_scale={}'.format(self.timestamp,self.x_dim1, self.x_dim2, self.y_dim1, self.y_dim2, self.alpha, self.beta, self.sd_y, self.df, self.scale)
        if not os.path.exists(self.graph_dir):
            os.makedirs(self.graph_dir)
        
        #save path for saving predicted data
        self.save_dir = 'data/bayes_infer/bayes_infer_{}_x_dim={}_{}_y_dim={}_{}_alpha={}_beta={}_sd={}_df={}_scale={}'.format(self.timestamp,self.x_dim1, self.x_dim2, self.y_dim1, self.y_dim2, self.alpha, self.beta, self.sd_y, self.df, self.scale)
    ...
    def bayesian_iteration(self, sample_size=50000, n_iters=3):
        ...
        y_prior, theta_prior = self.y_sampler.generate_data3(sample_size,5)
        self.y_true, self.theta_true = y_prior[0:10,:],theta_prior[0:10,:]
        np.savez('%s/theta_prior.npz'%self.save_dir,theta_prior,self.y_true,self.theta_true)
        data_y1 = y_prior
        data_y2 = theta_prior
        theta_posterior = self.train(data_y1,data_y2,0,epochs_list=[100,20])
        sys.exit()

        for i in range(1,n_iters):
            y_prior, theta_prior = self.y_sampler.generate_data3(sample_size,theta=theta_posterior)
            data_y1 = y_prior
            data_y2 = np.hstack((theta_prior,h_prior[:,block_size-1:block_size]))
            theta_posterior,h_last_posterior = self.train(data_y1, data_y2, i, epochs=1)
            #np.savez('%s/theta_posterior_%d.npz'%(self.save_dir,i),theta_posterior,self.y_true)
    ...
    def train(self,data_y1,data_y2,iteration,epochs_list):
        ...
        for epoch in range(epochs):
            ...
            for idx in range(batch_idxs):
                ...
                #update G 
                _ = self.sess.run(self.g_h_optim, feed_dict={self.x1: bx1, self.x2:bx2, self.y1: by1, self.y2: by2, self.lr:lr})
            ...
            #if (epoch+1) % 500 == 0:
            if (epoch>epochs-100 and (epoch+1)%5==0) or epoch+1==epochs:
                sample_size = data_y2.shape[0]
                #y1_block = self.y_true[0:1,iteration*self.y_dim1:(1+iteration)*self.y_dim1]
                ...
                for i in range(len(self.y_true)):
                    data_x1_, _ = self.predict_x(self.y_true[i:(i+1),:],data_y2[0:1,:])
                    x1 = np.tile(data_x1_[0,:],(sample_size,1))
                    x2 = self.x_sampler2.get_batch(sample_size)
                    _, y2_ = self.predict_y(x1,x2)
                    theta_post.append(y2_)
                y2_combine_ = np.concatenate(theta_post,axis=0)
                np.save('%s/theta_posterior_iter%d_epoch%d.npy'%(self.save_dir,iteration,epoch),y2_combine_)
        return y2_

        
    def theta_filter(self,theta_array):
        idx=[]
        for i in range(theta_array.shape[0]):
            theta = theta_array[i]
            if abs(theta[1])<1 and theta[2]>0:
                idx.append(i)
        return theta_array[idx],len(idx)
...
if __name__ == '__main__':
    ...
    g_net = model.Generator_Bayes(input_dim1=x_dim1,input_dim2=x_dim2,output_dim1=y_dim1,output_dim2=y_dim2,name='g_net',nb_layers=3,nb_units=32)
    h_net = model.Generator_Bayes(input_dim1=y_dim1,input_dim2=y_dim2,output_dim1=x_dim1,output_dim2=x_dim2,name='h_net',nb_layers=3,nb_units=32)
    ...
    dx_net2 = model.Discriminator(input_dim=x_dim1+x_dim2,name='dx_net2',nb_layers=2,nb_units=16)
    dy_net2 = model.Discriminator(input_dim=y_dim1+y_dim2,name='dy_net2',nb_layers=2,nb_units=16)
    ...
    RTM = RoundtripModel(g_net, h_net, dx_net1,dx_net2, dy_net1, dy_net2, xs1,xs2, ys, pool, batch_size, alpha, beta, sd_y, df, scale)
